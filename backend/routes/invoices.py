...
def add_invoice():
    ...
    # Validar si ya existe una boleta para esta orden
    existing_invoice = Invoice.query.filter_by(order_id=order_id).first()
    if existing_invoice:
        return jsonify({'error': 'Ya existe una boleta para esta orden'}), 409

    order = Order.query.get(order_id)
    if not order:
        return jsonify({'error': 'Orden no encontrada'}), 404

    # Construir ítems CON IGV (directamente como están guardados)
    items = []
    for item in order.order_items:
        items.append({
            ...
            'quantity': item.quantity,
            'price': float(item.unit_price),  # ya incluye IGV
        })
    ...
    pdf_bytes = generar_boleta_pdf({
        ...
        'customerPhone': order.shipping_address.get('phone', ''),
        'items': items,
    })
